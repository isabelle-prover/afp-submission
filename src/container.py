...
import config
import mailer
from entry import Entry, Result 
...
class Container():
    ...
    def execute(self):
        ...
        names = [e['shortname'] for e in self.entry.metadata.entries]
        ...
        with self.entry.open_archive() as af, \
            ...
            runner = IsabelleRunner(self.entry, af, names, l, cl)
        ...
        if self.entry.get_result() is Result.SUCCESS:
            ...
            # TODO: replace index.html with something more meaningful
            try:
